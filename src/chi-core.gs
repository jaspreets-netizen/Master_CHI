// ════════════════════════════════════════════════════════
// MASTER CHI SCORECARD v2.3 — CORE
// DO NOT MODIFY — this file is the stable v2.3 baseline.
// ════════════════════════════════════════════════════════
...
function buildMasterActivation() {
  ...
  var sh = ss.getSheetByName('Activation');
  ...
  sh.setColumnWidth(1, 30); sh.setColumnWidth(2, 180); sh.setColumnWidth(3, 160);
  sh.setColumnWidth(4, 500); sh.setColumnWidth(5, 200); sh.setColumnWidth(6, 200);
  ...
  sh.getRange(2, 1, 1, 6).setValues([['#', 'Site Name', 'CEM Name', 'CHI Sheet URL', 'Allow Access', 'Connection Status']])
  ...
  sh.getRange(3, 4).setValue('https://docs.google.com/spreadsheets/d/1FtBti0RsDJwuig1z71kdQL6rq6fKRFXUBHD4SF6d_RE/edit')
    .setBackground(CLR.ME).setFontSize(9).setWrap(true);
...
function getMonthColumns_() {
  ...
  var endMn = curMn + 1, endYr = curYr;
  if (endMn > 12) { endMn = 1; endYr++; }
...
function getBiweekColumns_() {
  var defs = [
    {mn:2,  bw:[[5,6],[7,8]]},
    {mn:3,  bw:[[9,10],[11,12],[13,14]]},
    {mn:4,  bw:[[15,16],[17,18]]},
    {mn:5,  bw:[[19,20],[21,22]]},
    {mn:6,  bw:[[23,24],[25,26]]},
    {mn:7,  bw:[[27,28],[29,30]]},
    {mn:8,  bw:[[31,32],[33,34],[35,36]]},
    {mn:9,  bw:[[37,38],[39,40]]},
    {mn:10, bw:[[41,42],[43,44]]},
    {mn:11, bw:[[45,46],[47,48]]},
    {mn:12, bw:[[49,50],[51,52]]}
  ];
  var today = new Date(), curYr = today.getFullYear(), curMn = today.getMonth() + 1;
  var endMn = curMn + 1, endYr = curYr;
  if (endMn > 12) { endMn = 1; endYr++; }
  ...
  while (yr < endYr || (yr === endYr && mn <= endMn)) {
    var def = null;
    for (var i = 0; i < defs.length; i++) if (defs[i].mn === mn) { def = defs[i]; break; }
    if (def) {
      for (var b = 0; b < def.bw.length; b++) {
        cols.push({label: 'Wk ' + def.bw[b][0] + ',' + def.bw[b][1], dashCol: idx + 2});
        idx++;
      }
    }
    mn++; if (mn > 12) { mn = 1; yr++; } if (mn === 1) continue;
...
function buildTrendTab_(tabName, labels, columns, titleSuffix, headerColor) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sh = ss.getSheetByName(tabName); if (sh) ss.deleteSheet(sh);
  ...
  for (var c = 2; c <= totalCols; c++) sh.setColumnWidth(c, 75);
  ...
  for (var i = 0; i < nc; i++) {
    sh.getRange(2, 2 + i).setValue(columns[i].label)
      .setBackground(CLR.DB).setFontColor(CLR.W).setFontWeight('bold').setHorizontalAlignment('center').setFontSize(9);
  }
  for (var s = 0; s < ns; s++) {
    var r = 3 + s, site = sites[s];
  ...
  if (ns > 0 && nc > 0) {
    var dataRange = sh.getRange(3, 2, Math.max(ns, 1), nc);
...
function buildCompleteCHIData_() {
  ...
  sh.getRange(1, 1, sh.getMaxRows(), totalCols).setFontFamily('Arial').setFontSize(11);
  sh.setColumnWidth(1, 140); sh.setColumnWidth(2, 160);
  for (var c = 3; c <= totalCols; c++) sh.setColumnWidth(c, 75);
  ...
  var hdr = ['Site', 'Score'];
  for (var i = 0; i < nc; i++) hdr.push(mCols[i].label);
  ...
  if (ns > 0 && nc > 0) {
    var dataRange = sh.getRange(3, 3, ns * stride, nc);
...
function buildAllTrends() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  ss.toast('Building trend sheets...', '⏳');
